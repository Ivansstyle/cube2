...
void CubeScene::draw() {
    ...
    glBegin(GL_POINTS); // Triangles take 3 verts
    for (i = 0; i < m_ntris; ++i) {
        for (j = 0; j < 3; ++j) {

            GLfloat step = (cos((m_rotation / 50.0f) * GLfloat(i / 100.0f - 5)));
            glVertex3f(m_verts[m_tris[i].idx[j]].x + step, m_verts[m_tris[i].idx[j]].y + step, m_verts[m_tris[i].idx[j]].z + step);
        }
    }
    glEnd(); // GL_TRIANGLES
    ...
    glColor3f(0.8f,0.8f,0.8f); // draw in black
    for (i = 0; i < m_ntris; ++i) {
...
void CubeScene::update() {
    ...
    GLfloat m_cosx = GLfloat(m_elapsedTime);
    // Use the elapsed time to do some sort of animation
    m_rotation = m_cosx * 50.0f;
}
